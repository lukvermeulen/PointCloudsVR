	...
	void LockBuffer(uint32 NewBufferSize)
	{
		if (NewBufferSize)
		{
			if (BufferSize < NewBufferSize || BufferSize > 1.2 * NewBufferSize)
			{
				...
				uint32 ExtendedBufferSize = NewBufferSize * 1.1;

				// Clip the size at UINT_MAX
				BufferSize = ExtendedBufferSize < NewBufferSize ? UINT_MAX : ExtendedBufferSize;
...
public:
	...
	virtual void Init(const FPointCloudVertexBuffer* VertexBuffer)
	{
		...
		{		
			ENQUEUE_RENDER_COMMAND(InitPointCloudVertexFactory)(
...
public:
	...
	static void ModifyCompilationEnvironment(const FVertexFactoryType* Type, EShaderPlatform Platform, const FMaterial* Material, FShaderCompilerEnvironment& OutEnvironment)
	{
		FPointCloudVertexFactory::ModifyCompilationEnvironment(Type, Platform, Material, OutEnvironment);
#endif
...
public:
	FPointCloudIndexBuffer() : NumPrimitives(0), Offset(0), BufferSize(0), Size(10000)
	{
	...
	void UpdateData(bool bUsesSprites)
	{
		...
		if (NewNumPrimitives != NumPrimitives)
		{
			...
			if (NewNumPrimitives)
			{
				...
				for (int32 i = 0; i < Offset; i++)
				{
					uint32 CacheSize = Nodes[i]->IBCache.Num() * sizeof(uint32);
					FMemory::Memcpy(Buffer, Nodes[i]->IBCache.GetData(), CacheSize);
					Buffer += CacheSize;
				}					
...
private:
	...
	UPointCloudPluginComponent *Component;
...
public:
	...
	void Initialize(UPointCloudPluginComponent* InComponent)
	{
...
{
public:
	FPointCloudSceneProxy(UPointCloudPluginComponent* Component) : FPrimitiveSceneProxy(Component)
	{
...
#else
							MeshBatch.MaterialRenderProxy = PointCloudProxy.GetMaterial(LOD)->GetRenderProxy();
#endif
...
FPrimitiveSceneProxy* UPointCloudPluginComponent::CreateSceneProxy()
{
...
void UPointCloud::FPointCloudRenderData::InitResources()
{
	...
	{
		ENQUEUE_RENDER_COMMAND(InitPointCloudRenderData)(
...
void UPointCloud::FPointCloudRenderData::ReleaseResources(EVBReleaseMode VBReleaseMode)
{
	...
	{
		ENQUEUE_RENDER_COMMAND(ReleasePointCloudRenderData)(
			[this, VBReleaseMode](FRHICommandListImmediate & RHICmdList)
			{
				ReleaseResources(VBReleaseMode);
			});
	}
}
