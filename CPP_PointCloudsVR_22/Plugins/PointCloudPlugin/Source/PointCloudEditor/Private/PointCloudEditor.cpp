...
void FPointCloudEditor::RegisterTabSpawners(const TSharedRef<class FTabManager>& InTabManager)
{
	WorkspaceMenuCategory = InTabManager->AddLocalWorkspaceMenuCategory(LOCTEXT("WorkspaceMenu_PointCloudEditor", "Point Cloud Editor"));
	auto WorkspaceMenuCategoryRef = WorkspaceMenuCategory.ToSharedRef();

	FAssetEditorToolkit::RegisterTabSpawners(InTabManager);

	InTabManager->RegisterTabSpawner(FPointCloudEditorTabs::DetailsID, FOnSpawnTab::CreateSP(this, &FPointCloudEditor::SpawnTab_Details))
		.SetDisplayName(LOCTEXT("DetailsTabLabel", "Details"))
		.SetGroup(WorkspaceMenuCategoryRef)
		.SetIcon(FSlateIcon(FEditorStyle::GetStyleSetName(), "LevelEditor.Tabs.Details"));
}

void FPointCloudEditor::UnregisterTabSpawners(const TSharedRef<class FTabManager>& InTabManager)
{
	FAssetEditorToolkit::UnregisterTabSpawners(InTabManager);

	InTabManager->UnregisterTabSpawner(FPointCloudEditorTabs::DetailsID);
}
...
void FPointCloudEditor::BindCommands()
{
	...
	PointCloudCommands->MapAction(
		FPointCloudEditorCommands::Get().ReimportCloud,
		FExecuteAction::CreateSP(this, &FPointCloudEditor::ExecuteCommand_Reimport));
}
	...
	struct Local
	{
		static void FillToolbar(FToolBarBuilder& ToolbarBuilder)
		{
			...
			ToolbarBuilder.BeginSection("Command");
			{
				...
				ToolbarBuilder.AddToolBarButton(PointCloudCommands.ReimportCloud, NAME_None, TAttribute<FText>(), TAttribute<FText>(), FSlateIcon(FEditorStyle::GetStyleSetName(), "TextureEditor.Reimport"));
			}
...
void FPointCloudEditor::ExecuteCommand_Reimport()
{
	if (PointCloudBeingEdited)
	{
		bool bSuccess = false;
		...
		if (PointCloudBeingEdited->ImportSettings.IsValid())
		{
		...
		if (bSuccess)
		{
			PointCloudBeingEdited->Reimport();
		}
	}
}
